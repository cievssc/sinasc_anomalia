  ...
  dados_all_hospital <- eventReactive(input$head_atualizar, {
...
output$nascido_graf_gestacao <- renderApex({
 ...
 mod_summary_card_server('nascido_tabela',
  ...
  output$nascido_mapa_rede <- renderLeaflet({ mapas    
        })

  observe({
    #req(!is.null(dados_nv()))
    dadoi <- dados_nv()

    dadoii <- dadoi[with(dadoi, (codmunres > 420000 & codmunres <= 429999) & (codmunnasc > 420000 & codmunnasc <= 429999)),]
   rede <- dadoii[!duplicated(dadoii[c('codmunnasc', 'codmunres')]) & !is.na(dadoii$codestab),]
   rede <- left_join(rede[,c('codmunnasc','codmunres')], centroide, by = c('codmunnasc' = 'cod6')) %>%
            left_join(., centroide, by = c('codmunres' = 'cod6'), suffix = c('.nasc','.res'))
   
   linha <- lapply(1:nrow(rede), 
                    function(i){sp::Lines(list(sp::Line(data.table::rbindlist(list(rede[i,3:4], rede[i,5:6])))), as.character(i))}) %>% sp::SpatialLines(.)

    
    leafletProxy('nascido_mapa_rede') %>%
        #addProviderTiles("OpenStreetMap.Mapnik") %>% leaflet()
        addProviderTiles(providers$CartoDB.Positron,  options = providerTileOptions(minZoom = 7)) %>%
        setView(lat = -27.5, lng = -51, zoom = 7)  %>% clearControls() %>% clearShapes() %>%
        addPolygons(data = municipiosf,  color = "#969090", #fillColor =  fill_color(dados_mapa$Freq)[[2]], 
        stroke = T, smoothFactor = 0.5, fillOpacity = 0.8, weight = 1.5,
    highlight = highlightOptions(
    weight = 5,
    color = "#666",
    ...
    addPolylines(data = linha,color = "red",
        weight = 1, opacity = 0.5, group = 'Rede de nascimentos') 
